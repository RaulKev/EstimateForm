...
import type { InsurancesData, PetQuoteRequest } from "@/features/estimate/type/insurance.types";
import { UploadFileType, type PurchasePolicyResponse, type UploadImageResponse } from "../types/pet-service.type";

export const generatePetQuote = async (insuranceData: PetQuoteRequest): Promise<InsurancesData> => {
  try {
    ...
    const isPendingQuote = data.status === 'pending' || !data.quoteNumber || data.quoteNumber <= 0;
...
export const uploadPetImage = async (
  ...
  try {
    ...
    const data = await httpClient.post<UploadImageResponse>(`/insurances/${insuranceId}/upload`, formData);
    return data;
  } catch (error) {
...
export const purchaseQuote = async (insuranceId: string): Promise<PurchasePolicyResponse> => {
  try {
    const { data } = await httpClient.post<PurchasePolicyResponse>(`/insurances/${insuranceId}/purchase`);
